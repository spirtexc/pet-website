...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_pet'])) {
    ...
    // Prepare the statement for inserting into pet_history
    $petHistoryStmt = $conn->prepare("
        INSERT INTO pet_history (PetID, PetName, Species, DateOfBirth, Weight, Allergies, delete_date, CustomerID)
        SELECT PetID, PetName, Species, DateOfBirth, Weight, Allergies, NOW(), CustomerID 
        FROM pets 
        WHERE PetID = ? AND CustomerID = ?
    ");

    // Bind the parameters (assuming both PetID and CustomerID are integers)
    $petHistoryStmt->bind_param("ii", $petID, $_SESSION['ID']);
    
    // Execute the pet history insertion
    if ($petHistoryStmt->execute()) {
        // Prepare the statement to delete the pet
        ...
        echo "Error inserting into pet history: " . $petHistoryStmt->error;
    }

    $petHistoryStmt->close();
}
