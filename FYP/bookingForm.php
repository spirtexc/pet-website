...
if ($customerID) {
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        // You can also store the pets if needed
        $pets = [];
        do {
...
<body>
    ...
    <?php 
    // Check if 'serviceid' exists in the GET request
    ...
    <script>
        $(document).ready(function() {
            let unavailableDates = []; // Dates that are fully booked
            let lowAvailabilityDates = []; // Dates with 1-2 slots left
            let availableDates = []; // Dates with more than 2 slots left
            ...
            function handleDateSelection(selectedDate) {
                // Use AJAX to fetch available time slots for the selected date
                $.ajax({
                    url: 'check-time.php',
                    method: 'POST',
